...
def create_app() -> FastAPI:
    ...
    def chat(req: ChatRequest) -> ChatResponse:
        ...
        reply = engine.reply(req.message, req.history, mode=req.mode)
        diag = (
            engine.last_diagnostics()
            if hasattr(engine, "last_diagnostics")
            else {"question": req.message, "reply_len": len(reply), "fallback": False}
        )
        record_debug(diag)
        return ChatResponse(
            reply=reply,
        ...
        def events():
            try:
                ...
                diag = (
                    engine.last_diagnostics()
                    if hasattr(engine, "last_diagnostics")
                    else {"question": req.message, "mode": req.mode}
                )
                record_debug(diag)
