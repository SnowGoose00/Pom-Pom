...
细节见 `SECURITY.md`。两份评审文档已从版本库摘出（留在磁盘、加进 `.gitignore`），不随发布上传。
...
1. **剧情覆盖**：Mission（主线／支线演出）+ 模拟宇宙 + 智库图鉴都已入库；剩余缺口主要是限时活动
   （`ExcelOutput/Activity*`、`Config/Level/EventMission` 等）与未被任何提取器引用的 NPC／道具文本。
2. **检索**：实体字面通道 + 拥挤治理（过取／MMR／单场景配额／专题预留席位）已上线。仍可改进的是实体抽取：
   目前靠规则切分，遇到「A 在 B 做了什么」这类句式会切出废词；彻底解决要上 FTS5 trigram + BM25 + RRF，
   以及一张更全的别名词表。
3. **联网检索**：已接入博查（带 key）作为首选源，免 key 的 DuckDuckGo / 360 / 搜狗退为兜底；
   未配 `BOCHA_API_KEY` 时自动只用免 key 源。
4. **回答里的 Markdown**：模型偶尔输出 `**加粗**`，前端用 `textContent` 渲染，用户会看到星号。
5. **深度模式更慢更贵**：一次提问 1–3 分钟、数倍 token；默认 6 轮 / 180 秒，可用 `DEEP_THINK_*` 调整。
6. **`insufficient` 语义偏保守**：它只表示「一条片段都没查到」（用于触发无答案协议）；
   「查到但都无关」由 `no_answer` 与回答文本本身体现。
7. **密钥管理**：真实 Key 只放在 `.env`（已 gitignore），换机器需重新配置；万一外泄请立刻在对应平台轮换。
